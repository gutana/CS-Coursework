...
void (*InputManager::anyKeyPressedCallback)() = 0;
...
void InputManager::keyboardDown(unsigned char key, int x, int y)
{
	...
	if (lower == 'w' || lower == 'a' || lower == 's' || lower == 'd' || lower == ' ' || lower == '/' || lower == 'q' || lower == 'e')
	{
		if (anyKeyPressedCallback != 0) anyKeyPressedCallback();
	}
}
...
void InputManager::RegisterMoveOrRotateKeyDownCallback(void(*callback)())
{
	anyKeyPressedCallback = callback;
}
