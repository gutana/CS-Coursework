...
static FishEntity* currentlyChasing;
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Assignment 5 - CS 409");
	...
	InputManager::RegisterKeyDownCallback('5', []() { lastNearestSchoolLookedAt->chooseNewExploreTarget(); });
	...
	InputManager::RegisterKeyDownCallback('z', []() {
		g_Camera.autopilotEnabled = !g_Camera.autopilotEnabled;
		std::cout << "Autopilot " << (g_Camera.autopilotEnabled ? "enabled.\n" : "disabled.\n");
	});

	InputManager::RegisterMoveOrRotateKeyDownCallback([]() {
		if (g_Camera.autopilotEnabled != false)
			std::cout << "Autopilot disabled.\n";
		g_Camera.autopilotEnabled = false;
	});
...
void init ()
{
	...
	for (int i = 0; i < loadInstructions.fishSchoolConfigurations.size(); i++)
	{
		FishSchoolEntity* school = new FishSchoolEntity(loadInstructions.fishSchoolConfigurations[i].center,
			...
			loadInstructions.fishSchoolConfigurations[i].fishAcceleration, &g_Terrain.hMapUnderWater);
...
void doAllPhysicsUpdates(double dt)
{
	...
	for (auto school : g_FishSchools)
	{
		...
		// While we're here, might as well do the school nearest neighbour updates
		school->UpdateNeighbours();
	}
	...
	if (lastNearestSchoolLookedAt != nullptr)
	{
			...
			lastNearestSchoolLookedAt->transform.scale.x))
		{
			...
			for (auto fish : fishCaught)
			{
				...
				if (fish == currentlyChasing)
				{
					currentlyChasing = nullptr;
				}
				if (fish->isFlockLeader)
				{
					lastNearestSchoolLookedAt->chooseNewFlockLeader();
				}
				delete(fish);
			}
...
FishSchoolEntity* findNearestSchoolToPosition(const Vector3& pos, const std::vector<FishSchoolEntity*>& objects)
{
	...
	float nearestDistance = pos.getDistanceXZ(nearest->transform.position) - nearest->transform.scale.x;
	...
	for (int i = 1; i < objects.size(); i++)
	{
		...
		float distanceToCurrent = pos.getDistanceXZ(current->transform.position) - current->transform.scale.x;
...
void drawUI() // todo: line things up better 
{
	...
	{
		std::stringstream ss;
		ss << (g_Camera.autopilotEnabled ? "[Z] Disable Autopilot" : "[Z] Enable Autopilot");
		g_Font.draw(ss.str(), g_WindowWidth / 2 - g_Font.getWidth(ss.str()) / 2, (g_WindowHeight / (float)2) + fishCaughtCounterOffsetFromCenter + 28, SpriteFont::BOLD);
	}

	if (g_Camera.autopilotEnabled)
	{
		std::stringstream ss;
		if (autoPilotState == AutopilotState::FarFromTargetSchool)
		{
			ss << "[Autopilot] Far from target school";
		}
		else if (autoPilotState == AutopilotState::NearSurface)
		{
			ss << "[Autopilot] Near surface";
		}
		else
		{
			ss << "[Autopilot] Near target";
		}
		g_Font.draw(ss.str(), g_WindowWidth / 2 - g_Font.getWidth(ss.str()) / 2, (g_WindowHeight / (float)2) + fishCaughtCounterOffsetFromCenter - 28, SpriteFont::BOLD);
	}
...
void printAutoPilotState(AutopilotState state)
{
	switch (state)
	{
	case AutopilotState::FarFromTargetSchool:
		std::cout << "Autopilot state: FarFromTargetSchool\n";
		break;
	case AutopilotState::NearSurface:
		std::cout << "Autopilot state: NearSurface\n";
		break;		
	case AutopilotState::NearTarget:
		std::cout << "Autopilot state: NearTarget\n";
		break;
	}
}

const float AP_SCHOOL_NEAR_DIST_THRESHOLD = 10.0f;
const float AP_SURFACE_DIST = 1.0f;

AutopilotState updateAutoPilotState(FishEntity* nearest)
{
	if (nearest->transform.position.getDistanceXZ(g_Camera.transform.position) < AP_SCHOOL_NEAR_DIST_THRESHOLD)
	{
		return AutopilotState::NearTarget;
	}

	if (g_Camera.transform.position.y > -1.0f)
	{
		return AutopilotState::NearSurface;
	}

	return AutopilotState::FarFromTargetSchool;
}

const unsigned int AP_STATE_UPDATES_EVERY_TICKS = 60;

void handleAutopilot()
{
	static unsigned int autopilotStateUpdateCount = 0;

	autopilotStateUpdateCount++;

	static FishSchoolEntity* nearest = nullptr;

	FishSchoolEntity* newNearest = lastNearestSchoolLookedAt;
	...
	if (nearest == nullptr)
	{
		std::cout << "Autopilot unable to find nearest school.\n";
		return;
	}

	if (currentlyChasing == nullptr)
	{
		currentlyChasing = nearest->children[0]; // we don't really know which fish is which. the 0th fish is effectively random to human beings
	}

	// think
	if (autopilotStateUpdateCount % AP_STATE_UPDATES_EVERY_TICKS == 1) //1 to make this to happen on first update
	{
		std::cout << "Autopilot updating state.\n";
		autoPilotState = updateAutoPilotState(currentlyChasing);
		printAutoPilotState(autoPilotState);
	}
	...
	{	
		case (AutopilotState::NearTarget):
		{
			...
			const float tuningParam = 1;

			float distBetweenEntities = g_Camera.transform.position.getDistance(currentlyChasing->transform.position) * tuningParam;
			float t = distBetweenEntities / g_MovementSpeed;

			Vector3 predictedPos = currentlyChasing->transform.position + currentlyChasing->velocity * t;
			Vector3 dirFromCamToTarget = predictedPos - g_Camera.transform.position;

			Vector3 targetVelocity = dirFromCamToTarget.getNormalized() * g_MovementSpeed;
			
			g_Camera.transform.rotateTowardPosition(predictedPos, g_LookSpeed * g_TimeManager->getUpdateDeltaTime());
			
			float angle = g_Camera.transform.forward.getAngleSafe((predictedPos - g_Camera.transform.position).getNormalized());
			if (angle < 0.01f)
			{
				Vector3 accel = g_Camera.transform.forward * PLAYER_ACCELERATION;
				g_Camera.velocity += accel;
			}
		...
		case (AutopilotState::NearSurface):
		{
			// Go towards target while staying near surface
			g_Camera.transform.rotateTowardPosition(
				{ currentlyChasing->transform.position.x, -0.5, currentlyChasing->transform.position.z }, // above us at -0.5f depth
				g_LookSpeed * g_TimeManager->getUpdateDeltaTime());

			float angle = g_Camera.transform.forward.getAngleSafe(((
				Vector3(currentlyChasing->transform.position.x, -0.5, currentlyChasing->transform.position.z) - g_Camera.transform.position))
				.getNormalized());

			if (angle < 0.01f && g_Camera.transform.position.y < 0)
			{
				Vector3 accel = g_Camera.transform.forward * PLAYER_ACCELERATION;
				g_Camera.velocity += accel;
			}
		...
		{
			g_Camera.transform.rotateTowardPosition(
				{ g_Camera.transform.position.x, -0.5, g_Camera.transform.position.z }, // above us at -0.5f depth
				g_LookSpeed * g_TimeManager->getUpdateDeltaTime());

			float angle = g_Camera.transform.forward.getAngleSafe(((
				Vector3(g_Camera.transform.position.x, -0.5, g_Camera.transform.position.z) - g_Camera.transform.position))
				.getNormalized());
